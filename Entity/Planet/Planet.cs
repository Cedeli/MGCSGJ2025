...
public partial class Planet : Node3D
{
	...
	// color
	[Export] public Color BaseColor = new Color("2a6f97");
	[Export] public Color SecondaryColor = new Color("6f7f69");
	[Export] public float ColorBlend = 0.5f;
	[Export] public float Roughness = 0.85f;
	[Export] public float Metallic = 0.05f;
	[Export(PropertyHint.Range, "1.0, 100.0, 0.5")] public float DetailNoiseScale = 30.0f;
	[Export(PropertyHint.Range, "0.0, 1.0, 0.01")] public float DetailNoiseIntensity = 0.15f;

	// atmosphere
	[Export] public bool HasAtmosphere = true;
	[Export] public Color AtmosphereColor = new Color("a9d6e5");
	[Export(PropertyHint.Range, "0.01, 0.5, 0.01")] public float AtmosphereThickness = 0.05f;
	[Export(PropertyHint.Range, "0.1, 5.0, 0.1")] public float AtmosphereDensity = 1.0f;
	[Export(PropertyHint.Range, "0.1, 10.0, 0.1")] public float AtmosphereFalloff = 5.0f;

	// water
	[Export] public bool HasWater = true;
	[Export(PropertyHint.Range, "-1.0, 1.0, 0.01")] public float WaterLevelOffsetFactor = -0.3f;
	[Export] public Color WaterColor = new Color("60a3bc");
	[Export(PropertyHint.Range, "0.0, 1.0, 0.01")] public float WaterSpecular = 0.5f;
	[Export(PropertyHint.Range, "0.0, 1.0, 0.01")] public float WaterRoughness = 0.1f;
	[Export(PropertyHint.Range, "0.0, 1.0, 0.01")] public float WaterTransparency = 0.7f;
	...
	public override void _Ready()
	{
		...
		if (_meshInstance == null)
		{
			...
			if (Engine.IsEditorHint()) _meshInstance.Owner = GetTree().EditedSceneRoot;
		}
	...
	{
		if (_rng == null) _rng = new RandomNumberGenerator { Seed = (ulong)GD.Randi() };
	...
	private void RandomizeNoise()
	{
		...
		NoiseOffset = new Vector3(_rng.RandfRange(-1000f, 1000f), _rng.RandfRange(-1000f, 1000f), _rng.RandfRange(-1000f, 1000f));
		HeightScale = _rng.RandfRange(0.03f, 0.12f) * Radius;
		if (_noise != null) _noise.Seed = (int)_rng.Randi();
	...
	private void RandomizeColors()
	{
		...
		float secHue, secSat, secVal, choice = _rng.Randf();
		if (choice < 0.4f) { secHue = _rng.RandfRange(0.05f, 0.15f); secSat = _rng.RandfRange(0.3f, 0.7f); secVal = _rng.RandfRange(0.3f, 0.7f); }
		else if (choice < 0.7f) { secHue = _rng.RandfRange(0.2f, 0.4f); secSat = _rng.RandfRange(0.2f, 0.6f); secVal = _rng.RandfRange(0.3f, 0.6f); }
		else { secHue = _rng.Randf(); secSat = _rng.RandfRange(0.0f, 0.15f); secVal = _rng.RandfRange(0.4f, 0.8f); }
	...
	public void GeneratePlanet()
	{
		...
		if (_noise == null) SetupNoise();

		_surfaceTool = new SurfaceTool();
		_surfaceTool.Begin(Mesh.PrimitiveType.Triangles);
		for (int i = 0; i <= Resolution; i++) {
			float lat = Mathf.Pi * (-0.5f + (float)i / Resolution);
			float sinLat = Mathf.Sin(lat); float cosLat = Mathf.Cos(lat);
			for (int j = 0; j <= Resolution; j++) {
				float lon = 2 * Mathf.Pi * (float)j / Resolution;
				float sinLon = Mathf.Sin(lon); float cosLon = Mathf.Cos(lon);
				Vector3 baseNormal = new Vector3(cosLon * cosLat, sinLat, sinLon * cosLat).Normalized();
				float height = GetNoiseHeight(baseNormal); Vector3 vertex = baseNormal * (Radius + height);
				float maxPossibleHeight = HeightScale; float minPossibleHeight = -HeightScale;
				float normalizedHeight = Mathf.InverseLerp(minPossibleHeight, maxPossibleHeight, height);
		...
		for (int i = 0; i < Resolution; i++) {
			for (int j = 0; j < Resolution; j++) {
				int topLeft = i * (Resolution + 1) + j; int topRight = topLeft + 1;
				int bottomLeft = (i + 1) * (Resolution + 1) + j; int bottomRight = bottomLeft + 1;
				_surfaceTool.AddIndex(topLeft); _surfaceTool.AddIndex(topRight); _surfaceTool.AddIndex(bottomLeft);
				_surfaceTool.AddIndex(topRight); _surfaceTool.AddIndex(bottomRight); _surfaceTool.AddIndex(bottomLeft);
			}
		}
	...
	private void GenerateOrUpdateAtmosphereMesh() {
		if (!HasAtmosphere) { if (_atmosphereInstance != null) _atmosphereInstance.Visible = false; return; }
		if (_atmosphereInstance == null) {
			_atmosphereMesh = new SphereMesh(); _atmosphereInstance = new MeshInstance3D { Name = "AtmosphereInstance", Mesh = _atmosphereMesh, MaterialOverride = _atmosphereMaterial, SortingOffset = 1.0f, CastShadow = GeometryInstance3D.ShadowCastingSetting.Off };
			AddChild(_atmosphereInstance); if (Engine.IsEditorHint()) _atmosphereInstance.Owner = GetTree().EditedSceneRoot;
		}
		_atmosphereInstance.Visible = true;
		float atmosphereRadius = Radius + (Radius * AtmosphereThickness);
		if (_atmosphereMesh != null) { _atmosphereMesh.Radius = atmosphereRadius; _atmosphereMesh.Height = atmosphereRadius * 2.0f; }
		UpdateAtmosphereMaterialParameters();
	}
	...
	private void GenerateOrUpdateWaterMesh()
	{
		if (!HasWater)
		{
			if (_waterInstance != null) _waterInstance.Visible = false;
			return;
		}
		...
		if (_waterInstance == null)
		{
			...
			_waterInstance.Name = "WaterInstance";
			_waterInstance.Mesh = _waterMesh;
			_waterInstance.MaterialOverride = _waterMaterial;
			_waterInstance.SortingOffset = 0.5f;
			_waterInstance.CastShadow = GeometryInstance3D.ShadowCastingSetting.Off;
			AddChild(_waterInstance);
			if (Engine.IsEditorHint()) _waterInstance.Owner = GetTree().EditedSceneRoot;
		}
	...
	private float GetNoiseHeight(Vector3 baseNormal) {
		if (_noise == null) { SetupNoise(); if (_noise == null) return 0f; }
		Vector3 point = baseNormal + NoiseOffset;
		float rawNoise = _noise.GetNoise3Dv(point);
		float height = rawNoise * HeightScale; return height;
	}
	...
	private void UpdateMaterialParameters() {
		...
		} else GD.PrintErr("Cannot update parameters as material is null");
	}
	...
	public Vector3 GetSurfacePosition(Vector2 uv) {
		float lon = uv.X * 2.0f * Mathf.Pi; float lat = Mathf.Pi * (-0.5f + uv.Y);
		float cosLat = Mathf.Cos(lat); Vector3 normal = new Vector3(Mathf.Cos(lon) * cosLat, Mathf.Sin(lat), Mathf.Sin(lon) * cosLat).Normalized();
		float height = GetNoiseHeight(normal); Vector3 position = normal * (Radius + height); return position;
	}
}
