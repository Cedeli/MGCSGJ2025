...
{
	[Export] private NodePath planetNodePath;
	[Export] private int polesLatitudeCount = 10;
	[Export] private int polesLongitudeCount = 20;
	[Export] private float poleHeight = 1.0f;
	[Export] private float poleRadius = 0.1f;
	[Export] private Color poleColor = Colors.Red;
	...
	public override void _Ready()
	{
		...
		if (!_regenerationTimer.IsConnected(Timer.SignalName.Timeout, Callable.From(OnTimerTimeout)))
		{
			Error err = _regenerationTimer.Connect(Timer.SignalName.Timeout, Callable.From(OnTimerTimeout));
			if (err != Error.Ok)
			{
	...
	private void PlacePoles()
	{
		...
		{
			_poleMaterial = new StandardMaterial3D
			{
				AlbedoColor = poleColor,
				Roughness = 0.8f
			};
		}
		...
		for (int i = 1; i < polesLatitudeCount; i++)
		{
			...
			for (int j = 0; j < polesLongitudeCount; j++)
			{
				...
				Vector3 surfaceNormal = (surfacePositionLocal / surfacePositionLocal.Length()).Normalized();
